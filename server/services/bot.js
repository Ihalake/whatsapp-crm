//Managing users + conversation memory talk to the database
const { v4: uuidv4 } = require("uuid");
const db = require("../db");
const { sendText, sendInquiryList } = require("./whatsapp"); //import from whatsapp.js

// --- Persistence helpers -> save/load data from DB
...
function findOrCreateLead(waPhone, profileName) {
  ...
  db.prepare(
    `INSERT INTO conversations (id, lead_id, state)
     VALUES (?, ?, 'awaiting_name')` //STATE MACHINE starting point.??
  ).run(uuidv4(), id);
